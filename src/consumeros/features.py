...
def load_profiles(source: Path | None = None) -> dict[str, FeatureProfile]:
    ...
    try:
        ...
        raise ConsumerOSError(f"Feature catalog could not be read: {exc}") from exc
    if payload.get("schema") != 1 or not isinstance(payload.get("profiles"), dict):
        raise ConsumerOSError("Feature catalog uses an unsupported format.")
    return {
...
def enable_feature(key: str) -> dict[str, Any]:
    ...
    if key not in profiles:
        raise ConsumerOSError(f"Unknown optional feature: {key}")
