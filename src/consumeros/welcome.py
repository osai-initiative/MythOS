...
WALLPAPER_ROOT = "file:///usr/share/backgrounds/consumeros/"
...
class Welcome(Adw.Application):
    def __init__(self) -> None:
        ...
        self.boot_action = next(
            (
                ...
                if item.startswith("consumeros.action=")
            ),
            "",
        )
        self.recovery = "consumeros.recovery=1" in cmdline or self.boot_action == "repair"
        self.boot_action_started = False

    @staticmethod
    def marker() -> Path:
        return Path.home() / ".config" / "consumeros" / "welcome-complete"
    ...
    def do_activate(self) -> None:
        ...
        if self.window:
            self.window.present()
            return
        self.window = Adw.ApplicationWindow(application=self, title="Welcome to ConsumerOS")
    ...
    def _live_page(self) -> Gtk.Widget:
        title = "ConsumerOS Recovery" if self.recovery else "Your computer, ready."
        subtitle = (
            "Repair, restore, reset, or reinstall an existing ConsumerOS system."
            if self.recovery
    ...
    def _installed_page(self) -> Gtk.Widget:
        ...
        wallpaper = Adw.ComboRow(
            title="Wallpaper",
            subtitle="A dark glass collection designed for ConsumerOS.",
        ...
        done = Gtk.Button(label="Start using ConsumerOS")
    ...
    def _launch_hub_recovery(self) -> bool:
        ...
        launcher.setenv("CONSUMEROS_START_PAGE", "recovery", True)
        try:
            launcher.spawnv(["consumeros"])
    ...
    def _hardware_check(self, button: Gtk.Button) -> None:
        ...
        def worker() -> None:
            result = subprocess.run(["consumerosctl", "--compact", "hardware"], capture_output=True, text=True, check=False)
            try:
    ...
    def _install_feature(self, button: Gtk.Button, feature: str) -> None:
        ...
        def worker() -> None:
            result = subprocess.run(
                ["pkexec", "consumerosctl", "--compact", "feature", "enable", feature],
                capture_output=True,
                text=True,
                check=False,
                timeout=7200,
