...
from .util import ConsumerOSError, atomic_write, file_lock, read_json, require_root, run, write_json


CURRENT_SOURCES = """# Managed by ConsumerOS. The stable base remains enabled in both channels.
Types: deb
...
def _lock_path() -> Path:
    return rooted("/run/lock/consumeros-update.lock")
...
def schedule_rollback(snapshot: int, transaction: str) -> Path:
    ...
    require_root()
    if snapshot <= 0:
        raise ConsumerOSError("Choose an existing system snapshot.")
    clean_transaction = re.sub(r"[^a-zA-Z0-9-]", "", transaction)[:64]
    if not clean_transaction:
        raise ConsumerOSError("The rollback transaction identifier is invalid.")
    snapshot_path = rooted(f"/.snapshots/{snapshot}/snapshot")
    if not snapshot_path.is_dir():
        raise ConsumerOSError("The selected system snapshot is no longer available.")
...
def configure_channel(channel: str) -> dict[str, str]:
    ...
    if channel not in {"stable", "current"}:
        raise ConsumerOSError("Update channel must be stable or current.")
    target = rooted("/etc/apt/sources.list.d/consumeros-current.sources")
...
def _create_snapshot(description: str, *, pre_number: int | None = None) -> int:
    ...
    try:
        ...
        raise ConsumerOSError("The system snapshot was created but its identifier was not returned.") from exc
...
def _set_system_update_link() -> None:
    ...
    if link.is_symlink():
        ...
        raise ConsumerOSError(f"{SYSTEM_UPDATE_LINK} exists and is not a symbolic link.")
...
def stage_update() -> dict[str, Any]:
    ...
    if rooted("/run/live/medium").exists():
        raise ConsumerOSError("Install ConsumerOS before applying system updates.")
    if not snapshotter_ready():
        raise ConsumerOSError(
            "Atomic updates require the ConsumerOS Btrfs layout and root snapshot configuration."
        )
    if rooted(ROLLBACK_REQUEST).exists():
        raise ConsumerOSError("Restart to finish the pending rollback before staging another update.")
    with file_lock(_lock_path()):
        ...
        snapshot = _create_snapshot(f"ConsumerOS pre-update {transaction[:8]}")
        state = {
            "status": "downloading",
        ...
        if not download.ok:
            ...
            raise ConsumerOSError(state["message"])
...
def cancel_staged_update() -> dict[str, Any]:
    ...
    with file_lock(_lock_path()):
        ...
        if state.get("status") not in {"downloading", "staged"}:
            raise ConsumerOSError("There is no staged update to cancel.")
...
def apply_offline_update() -> dict[str, Any]:
    ...
    with file_lock(_lock_path()):
        ...
        if state.get("status") != "staged":
            _remove_system_update_link()
            raise ConsumerOSError("No complete staged update is available.")
        ...
        if result.ok:
            post = _create_snapshot(f"ConsumerOS post-update {state['transaction'][:8]}", pre_number=snapshot)
            state.update(
                {
                    "status": "applied",
                    "post_snapshot": post,
        ...
        try:
            ...
            _append_history(dict(state))
            return state
        except ConsumerOSError as rollback_error:
...
def bless_boot() -> dict[str, Any]:
    ...
    if state.get("status") == "applied":
        ...
        state.update(
            {
                "status": "failed",
                "message": "Automatic rollback did not complete. Start ConsumerOS Recovery from the boot menu.",
            }
        )
        _write_state(state)
        _append_history(dict(state))
...
def update_status() -> dict[str, Any]:
    state = read_json(_state_path(), {})
    if not state:
        state = {"status": "never", "message": "No ConsumerOS update has run yet."}
