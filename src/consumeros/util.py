...
class ConsumerOSError(RuntimeError):
    """An actionable error safe to show in the graphical interface."""


class CommandError(ConsumerOSError):
...
def require_root() -> None:
    if os.geteuid() != 0:
        raise ConsumerOSError("Administrator approval is required for this action.")
...
def file_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise ConsumerOSError("Another system operation is already running.") from exc
        yield
