...
class SystemHub(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        self._install_css()
        self.window.set_title("ConsumerOS System Hub")
        self.window.set_default_size(1080, 720)

        outer = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        header = Adw.HeaderBar()
        title = Adw.WindowTitle(title="System Hub", subtitle="ConsumerOS")
        ...
        menu = Gio.Menu()
        menu.append("About ConsumerOS", "app.about")
        ...
        requested_page = os.environ.get("CONSUMEROS_START_PAGE", "")
    ...
    def _build_overview(self) -> Gtk.Widget:
        scroll, content = self._page("Your computer, ready.", "One place for the things that keep ConsumerOS dependable.")
        band = Gtk.Box()
    ...
    def _build_recovery(self) -> Gtk.Widget:
        ...
        migrate.add_suffix(self._button("Start", lambda _button: self._launch(["consumeros-migrate"])))
        group.add(migrate)
        ...
        reset_group = self._group("Reset or reinstall", "These actions run from ConsumerOS Recovery so personal files can stay unmounted and safe.")
    ...
    def _show_about(self, *_args: Any) -> None:
        ...
        about = Adw.AboutDialog(
            application_name="ConsumerOS",
            application_icon="org.consumeros.SystemHub",
            version="1.0.0",
            developer_name="ConsumerOS Build Team",
    ...
    def _run_ctl(
        ...
        *,
        ...
        command = (["pkexec"] if privileged else []) + ["consumerosctl", "--compact", *argv]
    ...
    def _confirm_reset(self, button: Gtk.Button) -> None:
        self._confirm("Reset desktop settings?", "ConsumerOS creates a backup first. Files and installed apps are not changed.", "Reset", lambda: self._run_ctl(["recovery", "reset-settings"], lambda _v: self._toast("Desktop settings reset."), button=button), destructive=True)
...
def drivers_main() -> int:
    os.environ["CONSUMEROS_START_PAGE"] = "hardware"
    return main()


def recovery_main() -> int:
    os.environ["CONSUMEROS_START_PAGE"] = "recovery"
    return main()
