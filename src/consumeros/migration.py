...
def _copy_file(source: Path, destination: Path) -> Path:
    if source.is_symlink():
        raise ConsumerOSError(f"Skipped symbolic link: {source}")
...
def build_plan(profile: WindowsProfile, target_home: Path, categories: Iterable[str]) -> list[MigrationItem]:
    ...
    import_root = target_home / "ConsumerOS Migration" / safe_name(profile.username)
    ...
    for item in item_list:
        ...
        try:
            if source.is_dir():
                ...
                copied += 1
                total_bytes += actual.stat().st_size
        except (OSError, ConsumerOSError) as exc:
