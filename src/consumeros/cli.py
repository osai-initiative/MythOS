...
from .util import ConsumerOSError, require_root, run
...
def _set_preference(key: str, value: str, acknowledge: bool = False) -> dict[str, Any]:
    ...
    if key == "diagnostics":
        ...
        if value == "enabled" and not acknowledge:
            raise ConsumerOSError("Enabling the macOS research provider requires acknowledging its compatibility warning.")
        config.macos_preview = value == "enabled"
    else:
        raise ConsumerOSError(f"Unknown preference: {key}")
    config.save()
    return {
        "key": key,
        "value": value,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="consumerosctl", description="ConsumerOS system control")
...
def dispatch(args: argparse.Namespace) -> Any:
    ...
    raise ConsumerOSError("Unknown command.")
