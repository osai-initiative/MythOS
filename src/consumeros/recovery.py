...
from .util import ConsumerOSError, is_device_path, require_root, run
...
def create_snapshot(description: str = "Manual restore point") -> dict[str, Any]:
    require_root()
    if not snapshotter_ready():
        raise ConsumerOSError("System snapshots are unavailable on this installation.")
    ...
    def walk(items: list[dict[str, Any]]) -> None:
        for item in items:
            if item.get("fstype") in {"btrfs", "ext4", "xfs"}:
                ...
                likely = label.upper() in {"CONSUMEROS", "COS_ROOT"} or any(
                    Path(mount, "etc", "consumeros").exists() for mount in mounts
                )
                candidates.append(
                    {
...
def reset_desktop_settings() -> dict[str, str]:
    """Back up and reset only ConsumerOS and GNOME desktop presentation keys."""

    home = Path.home()
    backup_dir = home / ".local" / "share" / "consumeros" / "backups"
...
def repair_bootloader(root_device: str, efi_device: str | None = None) -> dict[str, str]:
    """Repair GRUB only after validating an explicit ConsumerOS root device."""

    require_root()
    if not is_device_path(root_device):
        raise ConsumerOSError("The root target is not a valid device path.")
    if efi_device is not None and not is_device_path(efi_device):
        raise ConsumerOSError("The EFI target is not a valid device path.")
    fstype = run(["lsblk", "-no", "FSTYPE", root_device], check=True, timeout=20).stdout.strip()
    if fstype not in {"btrfs", "ext4", "xfs"}:
        raise ConsumerOSError("The selected root device does not contain a supported Linux filesystem.")

    mount_dir = Path(tempfile.mkdtemp(prefix="consumeros-repair-", dir="/run"))
    mounted: list[Path] = []
    try:
        mount_command = ["mount"]
        if fstype == "btrfs":
        ...
        marker = mount_dir / "etc" / "consumeros" / "release"
        os_release = mount_dir / "etc" / "os-release"
        if not marker.exists() and (not os_release.exists() or "ConsumerOS" not in os_release.read_text(errors="replace")):
            raise ConsumerOSError("The selected filesystem is not a ConsumerOS installation.")
        ...
        if efi_device:
            ...
            install = [
                ...
                "--bootloader-id=ConsumerOS",
                "--recheck",
            ]
        else:
            parent = run(["lsblk", "-no", "PKNAME", root_device], check=True, timeout=20).stdout.strip()
            if not parent:
                raise ConsumerOSError("The parent disk for the BIOS bootloader could not be identified.")
        ...
        return {"message": "The ConsumerOS bootloader was repaired.", "root": root_device}
    finally:
