...
from .util import ConsumerOSError
...
class CompatibilityDatabase:
    ...
    def reload(self) -> None:
        try:
            ...
            raise ConsumerOSError(f"Compatibility data could not be read: {exc}") from exc
        if payload.get("schema") != 1 or not isinstance(payload.get("entries"), list):
            raise ConsumerOSError("Compatibility data uses an unsupported format.")
