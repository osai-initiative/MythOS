...
from .util import ConsumerOSError, safe_name


APP_ID = "org.consumeros.Open"
...
def integrate_appimage(source: Path, applications_dir: Path | None = None) -> dict[str, str]:
    ...
    if not source.is_file() or source.is_symlink():
        raise ConsumerOSError("Choose a regular AppImage file.")
    with source.open("rb") as handle:
        header = handle.read(12)
    if len(header) < 11 or header[:4] != b"\x7fELF" or header[8:10] != b"AI":
        raise ConsumerOSError("This file does not contain a recognized AppImage header.")

    app_dir = Path.home() / ".local" / "share" / "consumeros" / "appimages"
    ...
    desktop = desktop_dir / f"consumeros-appimage-{digest}.desktop"
    content = (
        "[Desktop Entry]\n"
        "Type=Application\n"
        f"Name={display_name}\n"
...
class OpenApplication(Adw.Application):
    ...
    def _windows(self, path: Path) -> None:
        ...
        if entry.rating == CompatibilityRating.KNOWN_BROKEN:
            details.append("ConsumerOS will not start an app marked known broken unless its report is updated.")
        if not self.window:
            return
    ...
    def _offer_windows_setup(self) -> None:
        ...
        dialog = Adw.AlertDialog(
            ...
            body="ConsumerOS uses Bottles to isolate each app and manage Wine, DXVK, VKD3D, registry settings, and runtimes.",
        )
    ...
    def _install_windows(self) -> None:
        try:
            Gio.Subprocess.new(
                ["pkexec", "consumerosctl", "feature", "enable", "windows-apps"],
                Gio.SubprocessFlags.NONE,
            )
        except GLib.Error as exc:
    ...
    def _appimage(self, path: Path) -> None:
        ...
        dialog = Adw.AlertDialog(
            ...
            body="Portable apps are not reviewed by ConsumerOS. Only continue if you trust where this file came from. It will appear in the application launcher.",
        )
        dialog.add_response("cancel", "Cancel")
        ...
        def response(_dialog: Adw.AlertDialog, name: str) -> None:
            ...
            except (ConsumerOSError, OSError, GLib.Error) as exc:
    ...
    def _macos(self, path: Path) -> None:
        config = Config.load()
        runtime = next((item for item in (Path("/opt/consumeros/macos/bin/launch"), Path("/usr/bin/darling")) if item.exists()), None)
        if not config.macos_preview:
            ...
            return
        if not runtime:
            self._message("No compatible runtime is installed", "The research provider is enabled, but ConsumerOS does not claim a working macOS runtime on this system.")
            return
        if not self.window:
            return
        dialog = Adw.AlertDialog(
